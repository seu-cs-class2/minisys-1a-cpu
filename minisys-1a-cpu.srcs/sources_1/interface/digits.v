...
module digits (
  ...
  always @(posedge clk) begin  //写是上升沿
    if (rst == `Enable) begin
      sel_out <= 8'hff;
      data_out <= 8'hff;
    end else if(addr == 32'hfffffc00 && en == `Enable && ew == `Enable) begin //写的是显示数据寄存器 且使能有效
      case (data_in[7:0])
        //                GEF_DCBA
        4'h0: begin
      ...
      endcase
    end else if(addr == 32'hfffffc04 && en == `Enable && ew == `Enable)begin //写的是位数使能寄存器
  ...
  always @(*) begin
    if(rst == `Enable) begin
      data_out <= `ZeroWord;
    end else if (addr == 32'hfffffc00 && en == `Enable && ew ==`Disable) begin
      data_out <= {24'b000000,data_out};
    end else if (addr == 32'hfffffc04 && en == `Enable && ew == `Disable) begin
